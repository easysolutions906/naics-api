...
const PLANS = {
  free: { name: 'Free', lookupsPerDay: 50, batchLimit: 10, ratePerMinute: 10 },
  starter: { name: 'Starter', lookupsPerDay: 500, batchLimit: 50, ratePerMinute: 30 },
  pro: { name: 'Pro', lookupsPerDay: 5000, batchLimit: 100, ratePerMinute: 60 },
  business: { name: 'Business', lookupsPerDay: 25000, batchLimit: 200, ratePerMinute: 200 },
  enterprise: { name: 'Enterprise', lookupsPerDay: 100000, batchLimit: 500, ratePerMinute: 500 },
};
...
const createKey = (plan = 'pro', email = null, stripeCustomerId = null) => {
  const keys = loadKeys();
  const apiKey = `naics_${randomUUID().replace(/-/g, '')}`;
